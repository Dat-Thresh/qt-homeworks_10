...
{
    QByteArray sendedHeader;
    QDataStream streamOut(&sendedHeader, QIODevice::WriteOnly);

    streamOut << head;
    socket->write(sendedHeader);
...
void TCPclient::SendData(ServiceHeader head, QString str)
{
    ...
    QDataStream streamOut(&sendedData, QIODevice::WriteOnly);
    streamOut << head << str;
